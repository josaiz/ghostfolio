...
`WORKSHOP_DEMO_DATA` to be idempotent.
...
The solutions branch implements the **read-only via MCP** path as the functional happy path; frontend/backend are
delivered as **technical plans** generated by the commands.
